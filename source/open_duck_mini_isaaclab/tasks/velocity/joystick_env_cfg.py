...
class JoystickEnvCfg_V73(JoystickEnvCfg_V70):
    ...
    ## 처음 세운 전제는 틀렸다 — 심의 접지면은 TPU 밑창이 아니다

    처음엔 `foot_bottom_tpu.stl` 만 재고 "좌우 지지폭 16.3 mm 의 칼날" 이라고
    적었다. 그런데 **심이 실제로 딛는 면**을 링크 좌표계에서 다시 재니 다르다.
    발 링크의 콜리전 6 개 중 가장 낮은 것은 TPU 가 아니라 `l_foot_side` 이고,
    TPU 밑창은 그보다 **2.70 mm 위**라 지면에 닿지 않는다:

        기존      l_foot_side  z -43.00 mm   접지 1907 mm^2  99.3 x 38.7 mm
                  foot_bottom_tpu          지면 +2.70 mm  (안 닿음)
        big_foot  l_foot_side  z -43.00 mm   접지 2365 mm^2 108.6 x 42.9 mm
                  foot_bottom_tpu          지면 +0.76 mm  (여전히 안 닿음)

    즉 **심의 좌우 지지폭은 이미 38.7 mm 였다.** "16 mm 칼날" 은 심에 존재한
    적이 없고, 이 판에서 바뀌는 것은 접지 면적 +24 %, 좌우 폭 +11 % 뿐이다.
    기대했던 1152 -> 4225 mm^2 (3.7 배) 가 아니다.

    ## 그래서 이 판이 답하는 질문

    좁아진 질문이지만 여전히 유효하다: **좌우 지지 +11 % 와 발 질량 +10.4 g
    편측이 roll RMS·낙상률을 움직이는가.** 안 움직이면 발 크기는 원인이 아니다.

    ## 남은 진짜 쟁점 (실기 확인 필요)

    CAD 대로면 실기도 TPU 가 아니라 `l_foot_side` 로 딛는다. 만약 실기에서는
    TPU 가 아래로 튀어나와 있다면 (탄성체라 눌리거나 조립 공차) 실기 접지폭은
    16 mm, 심은 38.7 mm — **심실 간 좌우 지지가 2.4 배 어긋난다.** 그러면 심이
    실기보다 구조적으로 안정하게 나오는 것이 당연하고, 지금까지 심 순위와 실기
    순위가 어긋난 이유의 후보가 된다. 실물 발바닥에서 TPU 가 주변 쉘보다
    **튀어나왔는지 들어갔는지** 눈으로 확인할 것.
    ...
    **판정: 낙상률과 roll RMS 가 내려가는가.** 심에서 좌우 지지는 +11 % 뿐이라
    큰 변화를 기대하기 어렵다 — 안 내려가면 "발이 원인이 아니다" 가 아니라
    "심의 발은 이미 넓어서 이 실험으로는 못 가른다" 로 읽어야 한다.
    """
...
@configclass
class JoystickEnvCfg_V74(JoystickEnvCfg_V73):
    """imitation_v74 — 큰 발 + CoM -10 mm 위에 **토크·낙상·탐색 묶음**.

    2026-08-20, 사용자가 목표를 다시 세웠다: 피크 토크 최소화, 보행 효율
    최대화, 토르소 좌우 흔들림 최소화, 주행 안정성, robustness, 그리고
    "천천히라도 명령에 잘 따라 움직이는 것". **추종 정확도는 더 이상 1 순위가
    아니다** -- 명령을 따르기만 하면 되고, 대신 토크·효율·흔들림을 잡는다.

    기반은 v73 이다 (big_foot USD + CoM 뒤로 10 mm). 사용자가 그 둘을 유지한
    채로 이어 가라고 했다.

    ## 무엇을 거는가 -- v68 에서 효과가 확인된 묶음 그대로

        min_base_height_ratio  0.75 -> 0.85   살짝 무너져도 에피소드 종료
        torques_scale          -1.0e-3 -> -0.2
        init_noise_std         1.0 -> 1.3     (JoystickPPORunnerCfg_Explore13)

    셋은 v68 에서 함께 검증됐다. 새 지표로 다시 재 보니 효과가 뚜렷하다:

        버전   토크포화%   일률 W   roll속 deg/s
        v68      0.36      7.7      64.3
        v73      1.18     10.2      79.6
        v61      1.21      9.6      75.8

    포화율은 **|tau| 가 실효 한계 3.16 N·m 에 붙어 있는 시간 비율**이다.
    피크 값 자체는 모든 정책이 3.16 에 물려서 안 갈린다 -- 얼마나 자주
    물리는지가 실제 차이고, v68 이 3.3 배 낫다. 일률도 24 %% 적다.

    ## 대가

    v68 은 앞뒤 추종이 0.0054 -> 0.0195 로 나빠졌다. 예전 기준이면 탈락이지만
    지금 기준으로는 허용 범위다 -- 사용자가 "천천히라도 따라가면 된다" 고
    했다. 다만 **명령 방향으로 가지 않는 것**(부호가 틀리거나 정지하는 것)은
    허용되지 않는다. 6 방향 모두 부호가 맞는지 확인할 것.

    비교 기준 (v73 @3000): 점수 0.0269 · roll 4.71 · 낙상 1.2 %% ·
    포화 1.18 %% · 일률 10.2 W · roll속 79.6
    **판정: 포화율과 일률이 내려가는가.** 목표는 포화 <= 0.5 %%, 일률 <= 8.5 W.
    """
...
@configclass
class JoystickEnvCfg_V75(JoystickEnvCfg_V74):
    """imitation_v75 — v74 + **토르소 각속도 벌점**. 좌우 휘청거림을 직접 친다.

    2026-08-20. 사용자 목표 중 "걸을 때 좌우로 휘청거리지 않게" 를 겨냥한다.

    ## 왜 이 항인가 -- 이미 답이 나와 있었다

    `torso_ang_vel_scale` 은 v55(-0.7)·v60(-1.2)에서 이미 시험했고, 그때는
    추종이 나빠져서 버렸다. 그런데 **목표가 바뀌니 같은 데이터가 다르게
    읽힌다** -- 이 항을 켠 두 판이 흔들림·낙상에서 역대 최고다:

        버전   roll RMS   낙상 %%   roll속 deg/s   비고
        v55      2.64     0.0       53.4        torso_ang_vel -0.7
        v60      3.73     0.6       55.4        torso_ang_vel -1.2
        v68      4.05     1.1       64.3        (이 항 없음)
        v73      4.71     1.2       79.6        (이 항 없음)

    v55 는 **낙상 0.0 %%** 로 유일하게 2 순위 합격이었다. 버렸던 이유는
    추종(점수 0.0463)이었는데, 지금은 그게 1 순위가 아니다.

    -1.2(v60)가 아니라 **-0.7(v55)** 을 쓴다. 더 센 -1.2 가 흔들림을 더 잘
    잡지도 못했고(3.73 vs 2.64) 추종만 더 가져갔다.

    ## 겹침 주의

    v74 의 토크 벌점과 이 항은 둘 다 "덜 움직이는 쪽" 으로 민다. 합치면
    가만히 서 있으려 할 수 있다. `alive_scale`(20)과 추종 보상이 반대 힘이고,
    v55 에서 이 항 단독으로는 걷기가 유지됐다. **정지 명령이 아닌데 속도가
    0 에 가까우면 실패로 본다** -- 6 방향 v_base 부호를 반드시 볼 것.

    비교 기준 (v74). **판정: roll속과 roll RMS 가 내려가면서도 6 방향 부호가
    유지되는가.** 목표는 roll속 <= 60, roll RMS <= 3.5, 낙상 <= 0.5 %%.
    """
