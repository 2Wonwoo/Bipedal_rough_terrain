...
## v35 — 평지/자갈/자갈돌 혼합 지형 (2026-08-09, 진행 중)

사용자 요구: 실기가 자갈밭·험지에서도 잘 걷게 하고 싶다. v34c10(현재 최고,
정지 0.0058 · 추종 0.0166)까지는 평지 하나로만 학습돼 있었다.

### 바뀐 것 — v34c10 대비 지형 하나만

리워드 계수·네트워크 크기·imitation 로직·RSI·standstill_hold·hip_inward·
path_tracking — **전부 v34c10 그대로**. 한 번에 하나만 바꾼다는 이 로그의
원칙을 지키기 위해서다. 바뀐 것은 `terrain` 필드 하나뿐:

```
평지(plane) 단일 -> IsaacLab TerrainGeneratorCfg, 3m x 3m 타일 10x20 격자
    flat(평지)        50%
    gravel(HfRandomUniform, 높이 굴곡 4~20mm)  25%
    rocks(HfDiscreteObstacles, 높이 5~20mm)    25%
curriculum=False -> 난이도 점증이 아니라 매 iteration 마다 병렬 환경의
    절반은 평지, 절반은 험지에 동시에 서서 학습 (사용자가 명시적으로
    순차 커리큘럼 대신 이 방식을 선택함)
```

새 파일 `source/open_duck_mini_isaaclab/tasks/velocity/terrain.py`, 새 설정
`JoystickEnvCfg_Rough`, 새 태스크 `Isaac-OpenDuckMini-Joystick-Rough-v0`
(odm 별칭 `v35`/`rough`).

지형 스케일은 ANYmal급 표준 예제(IsaacLab `ROUGH_TERRAINS_CFG`, 노이즈
2~10cm)를 이 로봇 크기(2.7kg, 서 있는 높이 140~190mm, 발 길이 60mm — 약
4~5배 작음)에 맞춰 축소한 값이다.

### 같이 고친 버그

지형이 `generator` 모드가 되면 `env_origins`의 z가 더 이상 항상 0이 아니다
(자갈/돌 타일마다 원점 높이가 다름). `joystick_env.py`의 두 곳이 로봇의
world-frame z를 절대값으로 비교하고 있었다:

1. `_get_dones()`의 `collapsed` 판정 (쓰러짐 감지)
2. 비대칭 크리틱의 root height 관측 항

둘 다 `self._terrain.env_origins[:, 2]`를 빼도록 고쳤다. 평지(원점 z=0)에서는
계산값이 그대로라 **v1~v34c10 체크포인트·관측과 100% 동일 — 회귀 없음**.

### 검증

64 env x 3 iteration 스모크 테스트로 지형 생성 -> 리셋 -> 학습 스텝까지
에러 없이 도는 것 확인. 에피소드 길이가 평지 스모크 테스트와 비슷한 범위로
나와서(둘 다 학습 초반 미훈련 정책 기준) 지형 원점 높이 버그가 실제로
잡혔다는 간접 증거로 판단.

### 결과

두 번 돌렸다 — 처음엔 짧게(3000 iter) 파이프라인 검증 겸, 그 다음 오래
(20000 iter) 수렴 확인 겸. 둘 다 **환경 200개**로 돌렸다(v34c10은 4096개) —
GUI로 눈으로 보면서 돌리려고 일부러 줄인 것이라, 같은 iteration 수라도
v34c10보다 경험량이 약 20배 적다. **v34c10과 1:1로 비교할 실험이 아니라
파일럿이다.**

| | v34c10 (평지, 4096env, 수렴) | v35 @3000iter (200env) | **v35 @19999iter (200env)** |
|---|---:|---:|---:|
| 정지 속력 (m/s) | 0.0058 | 0.017 | **0.005** |
| 추종 평균 (전후좌우, m/s 오차) | 0.0166 | 0.044 | 0.044 |
| 5mm 위반율 | — | 0.0% | **0.3%** (앞 방향에서만, 최소 3.2mm) |
| 리워드 (학습곡선 마지막) | 426.2 | 148.4 | 237.0 |
| 에피소드 길이 (max 1000) | 778.7 | 355.6 | 500.4 |

**정지는 v34c10 수준까지 따라잡았다(0.005 vs 0.0058)** — standstill_hold가
험지에서도 그대로 작동한다는 뜻. **이동 중 추종은 3000iter 때와 거의 그대로
(0.044)** — 20000iter 동안 리워드·에피소드는 계속 늘었는데 추종 정확도 자체는
정체됐다. 험지에서 발이 미끄러지거나 걸리는 게 근본 원인으로 보이지만,
확인은 안 됐다.

**⚠️ 안전 지표가 3000iter 때보다 나빠졌다.** 3000iter 체크포인트는 전 방향
위반 0%(최소 6.1mm)였는데, 19999iter는 "앞" 방향에서 0.3% 위반(최소 3.2mm,
5mm 기준 미달)이 나왔다. 학습을 더 시킨다고 안전이 같이 좋아지는 게 아니라는
뜻 — 리워드가 추종·생존 위주로 최적화되는 동안 고관절 이탈 억제
(`hip_inward`)가 상대적으로 덜 우선시됐을 가능성이 있다. **실기 이식 전
반드시 재확인하고, 필요하면 v31/v32 때처럼 `hip_inward_thresh`를 더 죄어야
할 수 있다.**

### 진행 중 알아낸 것 (다음 학습 전 검토용)

정지 시 관절 떨림과 좌우(왼발/오른발) 비대칭이 사용자 관찰로 보고됨. 조사
결과:

* **떨림**: 이 프로젝트 자체 전례(`머리 3축 PID D=0->2000` 커밋)가 실기
  PID 감쇠 부족이 원인이었던 사례가 있다 — 지금 증상이 시뮬 상인지 실기
  상인지부터 구분해야 한다. 시뮬이라면 `action_rate_scale` 스윕, 관절
  가속도 페널티 신설, 그리고 `scripts/hw/rl_walk.py`의 사후 저역필터가
  이미 "학습 안 된 필터를 실기에서만 걸면 추종이 나빠진다"고 자인하고
  있으므로 **필터를 학습 루프 안에 넣는 방향**이 문헌상 더 낫다.
* **좌우 비대칭**: v29(`JoystickPPORunnerCfg_Symmetry`)가 미러 손실을
  시도했다가 실패한 전례가 있음 — 손실 항으로 유도하는 방식보다 최근
  문헌은 (a) 완전한 좌우 미러 데이터 증강(액터+크리틱 둘 다), (b) 대칭
  강제 네트워크 구조를 더 효과적이라고 본다. 단, RL을 손대기 전에
  레퍼런스 보행(`ref_g125.pkl`) 자체가 좌우 대칭인지부터 확인 필요 —
  비대칭이면 모방 리워드가 그걸 그대로 학습시킨다.

### 남은 것

* 안전 지표 회귀(0% -> 0.3%) 원인 규명 — iteration 늘릴수록 계속 나빠지는지
  중간 체크포인트들로 추이 확인 필요
* v34c10과 공정 비교하려면 같은 4096 env로 재학습 필요 (지금 둘 다 200env
  파일럿)
* 지형이 병렬 환경에 무작위 배정되므로, 같은 로봇이 매 에피소드 같은
  지형만 보는 게 아니다 — "이 정책이 자갈에서 특별히 강한가"를 따로 보려면
  `odm measure`를 평지 전용/험지 전용으로 나눠 돌릴 방법이 필요할 수 있다.
* 관절 떨림/좌우 비대칭 대응책(위 "진행 중 알아낸 것") 다음 학습에 반영
