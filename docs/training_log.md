...
### 폐루프 검증 — 목표 달성 (전 명령 5 mm 위반 0%)

정적 자세 검증만으로는 부족하다. 필터가 매 스텝 목표를 밀면 정책이 다음
관측에서 다르게 반응하므로 궤적 전체가 달라진다. 그래서 `JoystickEnvCfg_HipInwardSafe`
(v32 + 필터) 로 6 방향을 실제로 돌리고, 그 궤적을 정확 메시로 다시 쟀다.

| | v28 | v32 | **v32 + 필터** |
|---|---|---|---|
| 5 mm 위반 | 38.0 % | 1.0 % | **0.0 %** |
| 접촉 | 13.0 % | 0.0 % | **0.0 %** |
| 최소 간격 | 0.0 mm | 4.0 mm | **7.5 mm** |
| 간격 중앙값 | 6.7 mm | 13.8 mm | 14.3 mm |
| 추종 오차 | 0.0158 | 0.0230 | 0.0250 |

명령별로도 전부 0% 다. v32 단독에서 유일하게 남아 있던 회전 명령(6.0% 위반,
최소 4.0 mm)이 8.0 mm 로 올라갔다.

**추종 대가는 v32 대비 +9% 뿐이다** (0.0230 -> 0.0250). 필터가 개입할 일이
드물기 때문이고, v32 가 이미 99% 안전하게 걷고 있던 것이 여기서 값을 한다.
필터를 v28(위반 38%) 위에 얹었다면 훨씬 자주 개입했을 것이다 -- **정책을
안전하게 학습시키는 것과 필터로 보장하는 것은 대체재가 아니라 보완재다.**

### 실기 후보 구성

    정책    v32 model_2999   (안쪽 고관절 이탈 벌점으로 학습)
    필터    barrier_h5d.pt   (5 차원 장벽함수 + 반복 뉴턴 투영)
    보장    5 mm, 실측 최소 7.5 mm
    비용    추종 0.0158 -> 0.0250 (v28 대비 +58%, v32 대비 +9%)

### 실기 전 검증 3 항목 — 전부 통과

**1. 계산 시간** (`scripts/diag/bench_filter.py`). 20 ms 예산 대비:

    cpu  환경 1개   0.881 ms   4.4%
    cpu  환경 16개  1.748 ms   8.7%
    cuda 환경 1개   1.431 ms   7.2%

**GPU 가 필요 없다.** 오히려 CPU 가 빠르다 -- 망이 작아 커널 실행 오버헤드가
연산보다 크다. 온보드가 이 PC 보다 몇 배 느려도 여유가 있다.

**2. 외란**. 별도 실험이 필요 없었다 -- `push_robot` 이벤트는 cfg 에 정의돼
있고 `gait_compare.py` 가 끄지 않는다. 위의 폐루프 검증(위반 0%)이 **이미
외란이 켜진 상태의 결과**다. (지형은 여전히 평지다.)

**3. 시간적 연속성**. 필터가 목표를 급격히 바꿔 서보에 무리를 줄까 걱정했는데,
측정해 보니 **반대**다:

| | 관절 속도 평균 | p99 | 최대 |
|---|---|---|---|
| v32 | 69.6 deg/s | 251.0 | **422.7** |
| v32 + 필터 | 67.2 deg/s | 249.7 | **314.3** |

최대 속도가 422.7 -> 314.3 deg/s 로 줄었다. 위반 자세는 대개 관절이 크게
벗어난 순간이라, 그걸 되돌리는 것이 결과적으로 궤적을 완만하게 만든다.
스텝간 변화도 1.41 -> 1.36 도로 사실상 같다.

필터를 모터 속도 제한 **앞**에 배치한 것이 여기서 값을 한다 -- 필터가 민
목표도 속도 한계를 거치므로 서보에 무리한 명령이 나갈 수 없다.

### 남은 것

  - 다지형 (지금은 평지만)
  - 실기 이식 (ONNX 내보내기 + 온보드 통합)

---

## v34 계열 — 정지 안정화 + 레퍼런스 높이 스윕 (2026-08-07)

사용자 요구가 명확했다: **실기 배포 시 정지에서 가만히 서 있을 것**이 1순위,
추종이 2순위. 그 순서대로 실험했다.

### 출발점 — v33n 의 정지가 나빴다

새 CAD(2.7140 kg) + 새 레퍼런스(ref_g115)로 학습한 v33n 은 리워드·에피소드가
v32 보다 높았는데 **정지 명령에서 뒤로 0.019 m/s 흘렀다.** 원인을 코드에서 셋으로
좁혔다:

1. `reward_imitation` 은 정지에서 이미 완전히 꺼진다 (`* (cmd_norm > 0.01)`).
2. 그런데 관측의 `imitation_phase` 는 **명령과 무관하게 계속 돈다**.
3. 붙잡아 줄 것은 `cost_stand_still`(-0.2) 뿐이고 imitation(+0.19)의 1/30 이다.

정책이 "정지" 를 나타내는 안정된 입력을 못 받은 채 회전하는 위상 신호를 받고
있었던 것이다.

### 결과

| 지표 | v28 | v32 | v33n | v34c | v34c2 | **v34c10** | v34c20 |
|---|---:|---:|---:|---:|---:|---:|---:|
| **정지 속력** | 0.0146 | 0.0041 | 0.0192 | 0.0070 | 0.0139 | **0.0058** | 0.0162 |
| **추종 평균** | 0.0171 | 0.0190 | 0.0271 | 0.0236 | 0.0249 | **0.0166** | 0.0228 |
| 짧은 접촉 | 33.2% | 26.1% | 27.4% | 28.0% | 24.6% | 23.9% | 22.5% |
| 몸통 높이 | 141.1 | 128.6 | 129.8 | 130.6 | 130.3 | 139.4 | 149.9 |
| 리워드 | — | 372.1 | 399.5 | 380.7 | 401.0 | **426.2** | 381.5 |
| 에피소드 | — | 674.0 | 729.1 | 688.9 | 740.2 | **778.7** | 708.0 |

(정지 속력 = 정지 명령에서 실측한 몸통 속도의 크기, m/s. 4 env x 500 스텝,
과도구간 100 스텝 제외.)

**v34c — 위상 고정만.** 정지 0.0192 -> 0.0070 (2.7배). 추종도 덤으로 좋아졌다
(0.0271 -> 0.0236). 구조가 원인이었다는 확인.

**v34c2 — 거기에 `stand_still_scale` -0.2 -> -1.0.** 정지가 **오히려 나빠졌다**
(0.0070 -> 0.0139). 벌점이 자세 이탈뿐 아니라 관절 속도까지 누르니, 제자리로
되돌리는 보정 자체를 덜 하게 된 것으로 보인다. 에피소드는 740.2 로 올랐으니
"균형을 못 잡아서" 는 아니다. **계수 상향은 이 문제의 답이 아니다.**

**v34c10 — v34c + 레퍼런스 +10 mm (ref_g125).** 정지 0.0058, 추종 **0.0166**.
추종은 이 프로젝트 최고 기록이던 v28(0.0171)을 처음 넘었고, 리워드·에피소드·
채터링도 전부 최고다. 몸통 높이 139.4 mm 가 v28 의 141.1 mm 와 거의 같다 —
**추종 최고가 나오는 높이대가 따로 있다**는 v28 의 관찰이 재현됐다.

**v34c20 — +20 mm (ref_g135).** 정지 0.0162, 추종 0.0228 로 둘 다 나빠졌다.
149.9 mm 는 지나치다. v30(158 mm 에서 추종 47% 악화)과 같은 방향이다.

### 결론

**채택: v34c10.** 정지 위상 고정 + 몸통 139 mm.

높이-성능 관계가 단조롭지 않다: 130 mm(v34c) -> 139 mm(v34c10) 은 개선,
139 -> 150 mm(v34c20) 은 악화. **최적이 140 mm 근처에 있다.** v28(141.1)이
우연이 아니었던 셈이다.

### 남은 것

* 정지 0.0058 은 v32 의 0.0041 보다 아직 못하다. 다만 v32 는 옛 로봇
  (2.3388 kg) 이라 실기 배포 대상이 아니다.
* 좌측 게걸음만 0.0394 로 유일하게 v28(0.0181)보다 나쁘다. 좌우 비대칭의
  잔재로 보인다.
* 짧은 접촉(채터링) 23.9% — 레퍼런스는 2~3% 다. 착지 충격을 못 흡수한다는
  뜻이고, 아직 손대지 않았다.

## v35 — 정지 시 몸통 수직 (2026-08-08 ~ 08-09)

사용자 요구는 한 문장이었다: **"정지시 몸통 수직"**.

### 먼저: v34c10 은 학습할수록 앞으로 더 기울고 있었다

| 몸통 피치 (정지 명령) | @1500 | @2999 |
|---|---:|---:|
| v34c10 | +8.19° | **+14.73°** |
| v34u (정지 목표 관절각으로 우회) | +20.93° | — |
| **v35 (중력벡터 직접)** | **+1.28°** | **+0.63°** |

(`+` 가 앞으로 숙임. `scripts/diag/standstill_pose.py`, 16 env x 250 스텝,
과도구간 150 제외. 넘어짐은 셋 다 0.1%.)

아무 항도 몸통 기울기를 벌하지 않았으니 정책이 편한 자세로 흘러간 것이다.
"최고 성능" 으로 채택했던 v34c10 이 정지 **자세**로는 계속 나빠지고 있었다.

### 왜 v34u 가 실패했나

v34u 는 같은 목표를 **정지 목표 관절각**으로 풀었다 — FK 로 몸통이 수직이 되는
관절각을 구해 `cost_stand_still` 의 목표로 넣었다. 결과는 +8.19 -> +20.93 도로
**2.5배 악화**. 관절각은 몸통 기울기의 간접 손잡이다. 발바닥이 지면에 눕도록
물리가 몸통을 돌려버리면 목표 관절각을 맞춰도 몸통은 안 선다.

### v35 = `projected_gravity` 의 x,y 를 직접 벌한다

```python
cost_upright_standstill = (g_x**2 + g_y**2) * (cmd_norm < 0.01)   # scale -20
```

직립이면 `projected_gravity_b` 가 정확히 `(0,0,-1)` 이므로 x,y 가 그대로
기울기다. 어느 관절로 세울지는 정책이 알아서 고른다. **무릎을 더 굽히지 않고**
해냈다 — 정지 높이가 125.5 mm 로 v34c10 의 125.9 mm 와 같다.

### 결과

| 지표 | v34c10 | **v35** | |
|---|---:|---:|---|
| 정지 속력 (m/s) | 0.0058 | **0.0030** | 48% 개선 |
| 정지 몸통 피치 | +14.73° | **+0.63°** | 23배 개선 |
| 추종 평균 | **0.0166** | 0.0206 | 24% 악화 |
| 리워드 | **426.2** | 375.3 | |
| 에피소드 | **778.7** | 683.8 | |

방향별 (v35): 전진 0.018 · 후진 0.016 · 좌 0.038 · 우 0.025 · 회전 0.006.
좌측 게걸음이 여전히 유일한 약점이다 (v34c10 0.0394 와 사실상 같다) —
이 항과 무관한, 더 오래된 좌우 비대칭이다.

### 리워드가 낮아 보이는 것은 착시다

| | @1500 | @2999 |
|---|---|---|
| 리워드 v34c10 / v35 | 346.3 / 340.6 | 426.2 / 375.3 |
| 에피소드 v34c10 / v35 | 636.8 / 634.2 | 778.7 / 683.8 |
| **스텝당** v34c10 / v35 | 0.544 / 0.537 | 0.5473 / **0.5489** |

@2999 에서 `imitation`(0.2010/0.2005), `tracking_lin_vel`(0.1374/0.1371),
`torques`, `alive` 가 전부 사실상 같고 **스텝당 리워드는 v35 가 오히려 높다**.
총합 차이는 전부 에피소드 길이(-12%)에서 온다. 즉 리워드 곡선만 보고 "나빠졌다"
고 읽으면 틀린다 — 실제 비용은 **생존 12% 단축**과 **추종 24% 악화** 두 가지다.

로그의 `Episode_Reward/upright_standstill` 은 -0.0006 -> -0.0005 로 거의 안
움직이는 것처럼 보인다(`stand_still` 의 1/7). 정지 구간에서만 걸리는 항이라
전체 평균으로 희석돼 보이는 것이고, 실측 자세는 크게 바뀌었다. **이런 게이트된
항은 로그 크기로 세기를 판단하면 안 된다.**

### 판정

**정지를 먼저, 추종을 그다음** 이라는 우선순위(사용자 지시)에 맞다. 정지는 속력과
자세 **둘 다** 좋아졌고, 대가는 추종 24% 와 생존 12% 다. 실기 배포 목표가
"정지에서 가만히 서 있기" 이므로 v35 를 채택한다.

절충이 필요하면 다음 손잡이는 `upright_standstill_scale` 이다(-20). 낮추면 추종을
되찾고 자세를 일부 내주는 쪽으로 움직일 것이다. 다만 -20 에서 이미 0.63 도라
여유가 많으므로, 절반(-10)으로도 수직은 유지될 가능성이 높다.

### 참고: 로봇이 바뀌었다

v35 부터 로봇은 **2.7430 kg** 이다 (body tail 실측 113 g 을 밀도로 반영,
2.7140 -> 2.7430, CoM 뒤로 1.06 mm). USD 를 백업본과 대조해 질량 외에는 아무것도
안 바뀌었음을 확인했다. 1.07% 는 도메인 랜덤화 범위 안이지만 v34c10 과의 비교에
섞여 있다는 점은 감안할 것.

---

## v36 → v37 → v39 — 떨림을 필터에서 리워드로 옮기기

사용자 지시로 학습 방향이 셋으로 고정됐다.
**1) 보행성능(대칭성·추종성·모터부하) 2) 정지 시 토르소 수직 3) 외란극복.**
...
| `action_lowpass_alpha` (보행) | 0.5 | **0.0** | ← | ← |
| `action_lowpass_alpha_standstill` | 0.5 | **0.7** | ← | **0.0** |
| `hip_inward_walking_only` | False | ← | **True** | ← |
| `action_jerk_scale` | 0.0 | ← | ← | **−0.25** |

v38 은 39 iter 에서 중단하고 v39 로 합쳤다 (`..._v38_ABORTED_39iter`).
따라서 **v39 는 v37 대비 두 가지가 바뀌었다** — 필터 제거+진동벌점, 그리고
hip_inward 정지 게이트. 지표가 달라 귀속은 갈라서 읽을 수 있다.

### 측정 (v37 @2000 vs v39 @2600, 같은 스크립트·같은 인자)

| 지표 | v37 | v39 | |
|---|---|---|---|
| 추종 평균오차 (6방향) | **0.0120** | 0.0197 | ✗ +64% |
| 정지 몸통피치 평균 | **+2.38°** | +2.93° | ✗ |
| 정지 몸통피치 표준편차 | **3.36°** | 4.56° | ✗ |
| 정지 hip_roll 좌우 어긋남 | +4.67° | **+1.85°** | ✓ −60% |
| 보행 hip_roll 어긋남 | −2.58° | **−2.08°** | ✓ |
| 관절한계 초과 | 0.0% | 0.0% | = |
| 외란 0.75 m/s 넘어짐 | 21.9% | **18.8%** | ✓ |
| 외란 1.00 m/s 넘어짐 | **28.1%** | 37.5% | ✗ |
| 외란 1.50 m/s 넘어짐 | 90.6% | **79.7%** | ✓ |

### 떨림은 확실히 잡혔다

`action_rate` 는 계수가 모든 버전에서 −0.5 로 같아서 **버전 간 직접 비교가 되는
거칠기 계량기**다.

| | v35 | v36 (필터학습) | v37 | v39 |
|---|---|---|---|---|
| `Episode_Reward/action_rate` | −0.0291 | −0.0327 | −0.0304 | **−0.0167** |
| 정지 원시 액션요동 | 0.0537 | 0.0638 | — | **0.0390** |
| 정지 몸통각속도 | — | 0.2408 | — | **0.1457** |
| 전진 원시 액션요동 | — | 0.1821 | — | **0.1436** |

**필터로 학습한 v36 이 무필터 v35 보다 원시 액션이 더 거칠었다** — 필터가 정책을
게으르게 만든다는 가설의 직접 증거다. 진동 벌점으로 바꾼 v39 는 필터가 **하나도
없는데** 액션요동이 v36 대비 −39%, action_rate 비용이 v37 대비 −45% 다.
실기 떨림의 근본 대응으로 이 방향이 맞다.

원시 2차차분²/1차차분² = 0.0816/0.0334 = **2.44**. 순수 진동이면 4, 매끄러우면
0 이므로 아직 진동 성분이 남아 있다.

### 왜 추종이 나빠졌나 — 예산 문제

| | v37 | v39 |
|---|---|---|
| action_rate | −0.0304 | −0.0167 |
| action_jerk | — | −0.0204 |
| **매끄러움 합계** | **−0.0304** | **−0.0371** |

v39 가 매끄러움에 22% 더 쓴다. **1차차분은 "빠른 움직임"을 벌하고, 2차차분은
"진동만" 벌한다** (매끄러운 램프는 정확히 0). 진동을 jerk 가 맡은 이상
action_rate 는 이제 추종에 필요한 빠른 움직임까지 이중과금하고 있다.

### 새로 잰 것 — 모터 부하 (`scripts/diag/torque_load.py`, 최초 실행)

| | 정지 | 전진 |
|---|---|---|
| 14축 토크 제곱합 | 2.253 | **7.835** |
| 스톨(4.1 N·m) 도달 관절 | 없음 (최대 41%) | **knee L/R, hip_roll L/R = 100%** |
| 87% 이상 | — | ankle L 87.4% |

**보행 중 6축이 스톨 한계를 친다.** 99% 값은 2.2~3.2 N·m 라 상시가 아니라
과도구간이지만, 최대가 정확히 4.100 = `effort_limit_sim` 이라는 건 심에서
클리핑되고 있다는 뜻이다. 실기 XM430 이면 여기서 밀려 목표각을 못 따라간다.

그런데 `torques_scale = −1.0e-3` 이라 리워드 기여가 **−0.0002** — 사실상 무규제다.
사용자가 목표 1 에 "모터부하 포함"을 넣었으므로 이건 별도 실험으로 다룬다.

### 판정

v39 채택. 실기에서 실제로 문제였던 건 떨림이고 (교수님 보고 건), v39 가 필터
없이 그걸 가장 잘 잡았다. 추종 손해(0.0120→0.0197)는 15~20 cm/s 명령에서 약
0.8 cm/s 추가 오차로, 자신을 흔들어 부수는 것보다 값싸다.
hip_inward 정지 게이트도 예측대로 hip_roll 을 4.67→1.85° 로 줄였으니 유지한다.

**다음 (v40): `action_rate_scale` −0.5 → −0.25 한 가지만.**
판정 — 추종이 v37 수준(0.0120)으로 회복되면서 액션요동(0.0390)과 원시 2차차분이
안 나빠지는가. 나빠지면 jerk 계수 쪽(−0.25)이 과했던 것이므로 되돌리고 그쪽을 만진다.

---

## 2026-08-14 — 부드러운 걸음: 무엇이 진짜 개선이고 무엇이 리워드 해킹인가

사용자가 우선순위를 못박았다: **1 추종 · 2 부드러움 · 3 진동.** 그리고 발에
3 mm 고무패드를 붙인 뒤 "발이 미끄러진다", "토르소를 최대한 부드럽게, 다리
움직임도" 라고 했다.

### 관측이 뒤집혔다 — 발을 낮추는 게 아니라 붙잡아야 한다

v53 은 `foot_lift_scale = -3000` 으로 발 들림을 **낮추는** 쪽에 걸었다. 그
전제가 무너졌다. 패드를 붙이자 발이 바닥을 끌기 시작했고, 재 보니:

    스윙 여유    중앙값 5.8 mm · p90 10.6 mm
    roll 진폭    ±10 도 → 반스탠스 70 mm 에서 발 모서리가 12 mm 내려감

**roll 만으로 스윙 발이 바닥에 닿는다.** 패드 전에는 조용히 미끄러지던 것이
패드가 물면서 끌림으로 드러난 것이다.

### v53 의 roll 반감은 리워드 해킹이었다

v53(= v48 + `foot_lateral -10`)이 roll 진폭 20.7 → 11.0 도, roll RMS
5.86 → 2.57 도로 절반이 됐다. 보상은 318.8 → 305.8 로 오히려 낮아서, **보상만
봤으면 되돌렸을 것이다.** 걸음 품질 표를 보고 성공으로 판정할 뻔했다.

접지 지속시간을 재고서야 알았다:

|  | 스탠스 | 스윙 | 주기 | 100 ms 미만 접지 |
|---|---|---|---|---|
| v48 왼 | 280 ms | 160 ms | 440 ms | 20/64 |
| v48 오 | 340 ms | 180 ms | 520 ms | 5/53 |
| v53 왼 | 140 ms | 120 ms | 260 ms | 9/79 |
| **v53 오** | **80 ms** | **60 ms** | **140 ms** | **56/96** |

오른발이 **7 Hz 로 떨고 있다.** 레퍼런스 주기는 540 ms 다. 걸은 게 아니라
바닥에서 진동한 것이고, 발을 바닥 근처에서 잘게 떨면 좌우 이동이 최소가
되므로 `foot_lateral` 벌점을 그렇게 피한 것이다.

접지 전환 횟수(`contact_hz`)만으로는 못 본다 — 늘어난 건 보이지만 빠른
걸음인지 떨림인지 구분이 안 된다. **주기와 짧은 접지 비율**을 표에 상설로
넣었다.

같은 일이 `imitation_v12` 에도 있었다. 사용자가 발이 "진동하는것마냥" 떤다고
했고 `w_stance_violation` 이 그때 만들어진 답이다. 지금 1.0 인데 v53 이
뚫었다 — `contact_rew = (swing − 1.0 × stance_violation) × 2.0` 에서 계수가
1:1 이라 떨면 두 항이 상쇄되고, 남는 `foot_lateral` 절감이 순이득이 된다.

### 새 리워드 4 항 (v55)

| 항 | 계수 | 사용자의 어느 말인가 |
|---|---|---|
| `cost_foot_clearance` | −8000 (목표 20 mm) | "발이 미끄러진다" / "직선으로" |
| `cost_foot_slip` | −18 | "발이 미끄러진다" |
| `cost_torso_ang_vel` | −0.7 | "토르소를 최대한 부드럽게" |
| `cost_joint_accel` | −2.2e−5 | "다리 움직임도" |

`cost_foot_clearance` 는 수평 속도로 가중해 이·착지 순간이 자연히 빠진다.
`cost_joint_accel` 이 `action_rate/jerk` 와 다른 이유는 후자가 **정책 출력**의
매끄러움이고 PD 게인과 부하 뒤의 실제 다리 움직임과는 다른 양이기 때문이다.

계수는 `scripts/diag/probe_terms.py` 로 재서 정했다 — v53 1 차 시도가 무너진
것이 계수를 추정해서였다. 정지 열이 모두 0 근처임을 확인했다.

### 도구에 남긴 것

- `scripts/diag/probe_terms.py` — 새 항의 원시 크기를 재고 목표 비중 계수를 역산.
  Isaac Kit 이 부팅 뒤 stdout 을 가로채므로 결과를 파일로도 쓴다.
- `gait_quality.py` — **추종**(명령·관절)과 **걸음 주기·짧은 접지** 추가.
  우선순위 1 위인 추종이 표에 없어서 "부드러워졌지만 안 간다" 를 못 걸렀다.
- `play_fixed_cmd.py --track-csv` — `vel_x/y/z` 열 추가 (명령 추종 측정용).

### 교훈

**보상으로 걸음을 판정하지 말 것.** v53 은 보상이 낮은데 roll 이 좋았고,
그 roll 은 가짜였다. 세 층을 다 봐야 한다 — 보상, 걸음 품질 지표, 그리고
**그 지표가 물리적으로 말이 되는지**(주기 540 ms, 접지 100 ms 이상).

---

## 2026-08-17 — 흔들림과 추종은 이 로봇에서 동시에 못 잡는다

사용자가 "1 순위 2 순위 둘 다 잡을 때까지 개입 없이 자동으로 돌려" 라고 해서
합격선을 숫자로 못박고(`scripts/diag/scoreboard.py`) 다섯 가지 지렛대를
차례로 시험했다.

    합격선   점수 <= 0.0180 (앞뒤 3 : 회전 2 : 옆 1)  ·  roll RMS <= 5.50도

| 버전 | 지렛대 | 점수 | 앞뒤 | 회전 | 옆 | roll |
|---|---|---|---|---|---|---|
| v59 | (기준) | **0.0165** | 0.0153 | 0.0106 | 0.0263 | 6.66 |
| v60 | `torso_ang_vel −1.2` | 0.0304 | 0.0275 | 0.0353 | 0.0341 | **3.73** |
| v61 | `foot_lateral −12` | 0.0221 | 0.0155 | 0.0104 | 0.0534 | 4.97 |
| v62 | v61 + 옆명령 게이트 | 0.0223 | 0.0200 | 0.0081 | 0.0434 | 5.39 |
| v63 | 마찰 0.3~1.0 | 0.0294 | 0.0172 | 0.0543 | 0.0410 | 7.06 |
| v64 | 보간격 0.22 | 0.0303 | **0.0106** | 0.0785 | 0.0413 | 5.52 |

**다섯 개 전부 실패했고, 실패 방식이 전부 달랐다.** 그게 결론의 근거다 —
계수 튜닝이 모자란 게 아니라 각 지렛대가 물리적으로 다른 것을 대가로 낸다.

    몸통 각속도 감쇠   증상을 직접 누르니 명령 추종의 자유도까지 묶인다
    발 좌우 억제       옆으로 걸으려면 발이 옆으로 가야 한다 (기하)
    마찰 확대          회전이 접지력에 가장 의존한다 — 5 배 악화
    넓은 스탠스        요 관성이 커지고 발이 회전축에서 멀어진다 — 7.4 배 악화

v59↔v60 두 점의 기울기로 산술 확인도 된다: roll 을 5.50 으로 낮추려면 계수
c~0.57 이 필요한데 점수 합격선은 c~0.13 에서 이미 넘어간다.

### 부산물로 얻은 것

  * **v64 의 앞뒤 0.0106 은 측정한 전 버전 중 최고다.** 넓은 스탠스가 전후
    추종에는 유리하다 — 회전만 아니면 쓸 값이다.
  * v64 는 READY 무릎을 -76.7 -> -66.7 도로 펴서 **실기 왼무릎 사각지대
    (-110 도)에서 10 도 더 멀어진다.** 나중에 그 문제가 다시 나오면 쓸 카드다.
  * v61 은 앞뒤·회전을 v59 와 **똑같이 유지**하면서 흔들림만 25 % 줄인다.

### 판정

**"둘 다"는 이 로봇의 물리적 프론티어 밖이다.** 고르는 문제로 바꾼다.

사용자의 순위(앞뒤 > 회전 > 옆걸음)와 실제 불만(흔들림)을 같이 보면
**v61 이 답이다** — 상위 두 방향을 그대로 두고 가장 낮은 항목 하나만
내주면서 흔들림을 25 % 줄인다. 종합 점수가 탈락시킨 이유는 옆걸음에
가중 1 을 주기 때문인데, 사용자가 불만인 것은 옆걸음이 아니다.

**교훈: 합격선을 스스로 세웠으면 그것이 실제 요구와 어긋날 때 숫자가 아니라
요구를 따라야 한다.** 다만 어긋났다는 것을 데이터로 보인 뒤에.

---

## 2026-08-20 — 리워드 감사: 18 개 중 6 개만 일한다

사용자: "지금 리워드함수가 너무 덕지덕지인지 아닌지 봐줘. 디즈니 논문에선
매우 간략하잖아, 또한 open_duck_mini repo 도. 근데 우리가 삽질하고 있는 게
아닐까 싶어서."

v70 에서 항목별 기여를 실측했다 (스텝당, 합 약 0.554):

| 항 | 기여 | 비중 |
|---|---|---|
| imitation | +0.2189 | **39 %** |
| path_tracking | +0.1782 | **32 %** |
| tracking_lin_vel | +0.1333 | **24 %** |
| alive | +0.0600 | 11 % |
| action_jerk | −0.0266 | −4.8 % |
| tracking_ang_vel | +0.0113 | 2 % |
| action_rate | −0.0111 | −2 % |
| hip_inward | −0.0052 | −0.9 % |
| stand_still | −0.0034 | −0.6 % |
| upright_standstill | −0.0006 | −0.1 % |
| leg_symmetry | −0.0002 | −0.04 % |
| torques | −0.0002 | −0.04 % |
| foot_lift · foot_lateral · foot_clearance · foot_slip · foot_impact · torso_ang_vel · joint_accel | 0.0000 | **꺼짐** |

**정의된 18 개 중 실제로 일하는 것은 6 개다.** 5 개는 1 % 미만이고 7 개는 0 이다.

### 삽질이 맞았다 — 다만 코드가 아니라 시간이

작동하는 계보(v42 → v59 → v65 → v70 → v73)는 처음부터 끝까지 **같은 9 개
설정**을 쓴다. v53~v69 에서 추가한 8 개 리워드 항은 **전부 실패했고 전부
꺼져 있다.** 그 실험들에 GPU 20 시간쯤 썼다.

성과를 낸 것은 전부 리워드가 아니었다:

    회전 6.3 배 회복      액션지연 범위 (0~3 vs 2~3)
    앞뒤 회복             질량 무작위화 범위 (0.90~1.10)
    추종·흔들림 동시 1 등  기존 항의 **가중치 재조정** (ang_vel_xy 0.1 -> 1.0)
    낙상 절반             CoM 위치 (모델)
    (진행 중)             발 접지면 (기구)

리워드를 **더한** 실험은 8 번 전부 실패했고, 구조·모델·기구를 바꾼 실험은
대부분 성공했다. 이 프로젝트가 스스로 적어 둔 교훈("구조 > 리워드 계수")이
데이터로 재확인됐다. 디즈니 논문과 원본 리포가 간결한 것은 우연이 아니다 —
**우리 활성 리워드도 6 개로 이미 간결하다.**

### 남은 큰 의심

`path_tracking` 이 32 % 인데 **실기에서는 그 관측이 상수 [0,1,0] 이었다.**
리워드의 3 분의 1 이 실기에 없는 정보로 학습됐다. 2026-08-18 에 `--path-imu`
로 방향 성분은 자이로 적분으로 복원했지만 **횡방향 오차는 여전히 0** 이다.
지금 남은 sim2real 갭 중 가장 큰 것일 수 있다.

### 도구

`scripts/diag/versions.py` — cfg 독스트링에서 `docs/versions.md` 를 생성한다.
v25 를 넘어가며 번호만으로는 무엇을 시험했는지 알 수 없게 됐다. 따로 관리하는
목록은 반드시 어긋나므로 생성한다.

---

## 2026-08-20 — 목표가 바뀌었다. v73 ~ v76

### 사용자가 다시 세운 우선순위

    1 피크 토크 최소화
    2 보행 효율 최대화
    3 토르소 움직임 최소화 (좌우 휘청거림)
    4 주행 안정성 · robustness
    5 명령 추종 — **천천히라도** 명령대로 가면 된다

추종 정확도가 1 순위에서 5 순위로 내려갔다. 이게 단순한 가중치 변경이 아닌
이유는, **예전 기준으로 버린 정책이 새 기준으로는 최고**이기 때문이다.
실제로 v55 (`torso_ang_vel -0.7`, 추종 점수 0.0463 으로 꼴찌라 버렸던 판)가
낙상 0.0 % 로 역대 유일이었고, 그 항이 v75 의 핵심이 됐다.

### 판정 지표를 먼저 다시 세웠다

재지 않으면 달성했는지 알 수 없다. `scripts/diag/scoreboard.py` 에 추가:

    포화%    |tau| 가 실효 한계 3.16 N·m 에 붙어 있는 시간 비율
    일률W    |tau·omega| 합의 평균 [W]
    roll속   roll 각속도 RMS [deg/s]

**피크 토크는 값 자체로는 지표가 안 됐다.** 7 개 정책 전부 p99 가 3.16 으로
같다 — 한계에 상시 물려 있다는 뜻이라, 얼마나 자주 물리는지로 바꿨다.
이걸 안 바꿨으면 "피크 토크 최소화" 를 측정할 방법이 아예 없었다.

### v73 — 큰 발. 전제가 틀려서 실패했다

`big_foot` USD (onshape 재수출) + CoM −10 mm. 접지면을 1152 -> 4225 mm^2 로
키운다는 것이 목표였다.

**학습 전에 재보니 전제가 틀렸다.** 발 링크 콜리전 6 개 중 가장 낮은 것은
TPU 밑창이 아니라 `l_foot_side` 이고, TPU 는 그보다 2.70 mm 위라 지면에
닿지 않는다:

    기존      l_foot_side  접지 1907 mm^2   99.3 x 38.7 mm   (TPU 지면 +2.70 mm)
    big_foot  l_foot_side  접지 2365 mm^2  108.6 x 42.9 mm   (TPU 지면 +0.76 mm)

즉 **심의 좌우 지지폭은 이미 38.7 mm 였다.** "16 mm 칼날" 은 TPU 단독 치수였고
심에 존재한 적이 없다. 이 판이 실제로 바꾼 것은 면적 +24 %, 좌우 +11 % 뿐이다.

결과: 점수 0.0269 · roll RMS 4.71 (v70 4.32 에서 악화) · 낙상 1.2 % (0.9 에서
악화). **판정 FAIL** — 다만 "발이 원인이 아니다" 가 아니라 "이 실험으로는 못
가른다" 로 읽어야 한다.

> 실기가 TPU 로 딛는지 `l_foot_side` 로 딛는지가 아직 미확인이다. 실기가 TPU
> 라면 실기 16 mm vs 심 38.7 mm 로 **좌우 지지가 2.4 배 어긋난다.** 심 순위와
> 실기 순위가 계속 어긋난 이유의 유력 후보다.

### v74 — v73 + v68 묶음. 토크·효율 역대 최고

    min_base_height_ratio  0.75 -> 0.85
    torques_scale          -1.0e-3 -> -0.2
    init_noise_std         1.0 -> 1.3

| | 포화% | 일률W | 점수 | 앞뒤 | rollRMS | 낙상% | roll속 |
|---|---|---|---|---|---|---|---|
| v73 | 1.18 | 10.2 | 0.0269 | 0.0241 | 4.71 | 1.2 | 79.6 |
| v74 | 0.31 | 7.3 | 0.0140 | 0.0070 | 5.05 | 1.1 | 80.0 |

토크 포화 3.8 배, 일률 −28 %. **예상했던 추종 대가가 안 나왔다** — v68 은
같은 묶음으로 앞뒤가 0.0054 -> 0.0195 로 망가졌는데, big_foot + CoM −10 기반
에서는 오히려 0.0241 -> 0.0070 으로 좋아졌다. 토크 벌점의 대가는 기반에 따라
달라진다.

토르소는 그대로였다 (roll속 79.6 -> 80.0). 토크 벌점은 흔들림을 안 건드린다.

### v75 — v74 + `torso_ang_vel -0.7`. 목표 1~4 번을 전부 잡았다

    포화 0.20 %  ·  일률 6.6 W  ·  roll속 46.2  ·  rollRMS 2.85  ·  낙상 0.2 %

전부 역대 최고다. **p99 토크가 3.12 로, 한계 3.16 에 닿지도 않는 첫 정책**이다.

6 방향 부호 전부 통과 (전진 0.111/0.15 · 후진 0.145/0.15 · 회전 0.968/1.0).
토크 벌점과 토르소 벌점이 겹치면 "안 움직이는 쪽" 으로 도망갈 수 있다고 봤는데
그러지 않았다. 추종 점수는 0.0275 로 v74 보다 나쁘지만 기준이 "천천히라도" 다.

> 주의: 이 수치는 **model_2800** 이다. 학습이 iter 2875/3000 에서 외부로
> 중단돼 `model_2999.pt` 가 없다. 93 % 지점 측정이라는 뜻이다.

`-1.2`(v60) 가 아니라 `-0.7`(v55) 을 쓴 것은, 더 센 값이 흔들림을 더 잘 잡지도
못하면서 (3.73 vs 2.64) 추종만 더 가져갔기 때문이다.

### 합격선을 통째로 교체했다

옛 기준(추종 점수 <= 0.0180 이 1 순위)으로 채점하니 **목표를 가장 잘 달성한
v75 가 떨어졌다.** 판정이 목표와 어긋나면 자동 루프가 엉뚱한 걸 쫓는다. 새 기준:

    포화 <= 0.30 % · 일률 <= 7.0 W · roll속 <= 50 · rollRMS <= 3.50 도
    낙상 <= 0.5 % · 6 방향 부호 OK

값은 v75 가 실제로 낸 수치 근처다 — **달성 가능함이 확인된 선이어야 목표로
쓸모가 있다.** 추종은 정확도가 아니라 방향 준수만 문턱으로 쓴다.

### v76 — robustness (진행 중)

목표 5 개 중 남은 것이 robustness 하나뿐이라, 정책도 리워드도 안 건드리고
환경만 험하게 한다:

    마찰            0.5~1.0 -> 0.4~1.3      실기 바닥 mu > 0.38, 지금은 미끄러운
                                            쪽을 아예 안 겪는다
    액추에이터 강성   0.9~1.1 -> 0.8~1.2      개체차·발열
    외란            +-1.0 -> +-1.5 m/s, 5~10 s -> 3~8 s

질량(0.9~1.1)은 안 건드린다. v58/v59 에서 넓혔다가 전진이 무너진 전례가 있고
실기 질량을 아직 안 쟀다.

**판정**: 넓힌 랜덤화 아래에서 v75 의 수치가 얼마나 무너지는가. 조금 나빠지는
것은 정상이다 (더 어려운 문제를 푼 것이므로). 크게 무너지면 마찰부터 되돌린다.

### 이번에도 도구 버그가 결과를 가리고 있었다

- **`--stream` 이 한 번도 뜬 적이 없었다.** `--kit_args <값>` 을 띄어 쓰면
  argparse 가 `--` 로 시작하는 값을 옵션으로 오인해 죽는다. `--kit_args=` 로
  붙여야 한다. 시그널링 49100 / 미디어 47998 로 고정도 같이 했다.
- **죽은 cfg 를 지우는 정리 커밋이 `READY_JOINT_POS_H175_ZNECK` 까지 지웠다.**
  쓰는 쪽은 남아 있어서 `joystick_env_cfg` **모듈 import 자체가 깨져 있었다** —
  학습·재생·측정이 전부 죽는 상태였는데 반나절 몰랐다. pyflakes F821 을
  `tests/test_no_undefined_names.py` 로 박아 뒀다.
- **녹화가 6 방향 중 2.6 개만 담겼다.** `--hold` 는 시뮬 시간인데 `--seconds`
  가 벽시계였고, 녹화 중 실시간 배속이 0.42 배라 50 초가 시뮬 21 초였다.
  종료 조건을 스텝 기준으로 바꿨다.

---

## 2026-08-21 — 장기 학습과 LPF. 그리고 도구 버그 하나 더

### 요약: 오늘 뒤집힌 판단 세 가지

    "3000 iter 면 수렴이다"      -> 아니다. 리워드는 평평한데 실제 지표는 계속 좋아진다
    "LPF 0.3 이 공짜 개선이다"    -> 덜 학습된 정책에서만 그렇다. 장기 학습이 대체한다
    "재개는 잘 되고 있다"         -> 지금까지 전부 조용히 처음부터 돌고 있었다

### 재개가 조용히 무시되고 있었다

`odm train <ver> <n> <envs> --resume` 이 체크포인트를 안 불러왔다. 오류도 안 났다.
재개했다는 런이 std 1.3 · 리워드 0.17 · 에피 24 로 시작하는 것을 보고 발견했다.

원인은 IsaacLab `cli_args.py:76` 이다:

    if args_cli.resume is not None:
        agent_cfg.resume = args_cli.resume

`--resume` 는 `action="store_true"` 라 기본값이 None 이 아니라 **False** 다. 즉
플래그를 안 주면 이 줄이 항상 False 로 덮어쓴다. odm 은 `agent.resume=true` 를
hydra 오버라이드로 넣고 있었는데, hydra 가 켠 값을 그 뒤에 도는
`update_rsl_rl_cfg` 가 지운 것이다. odm 주석의 "train.py 에 --resume CLI 는
없다" 도 사실이 아니었다.

고친 뒤에는 로그에 `Loading model checkpoint` 가 찍히고 std 가 0.20 에서
시작하는 것으로 작동을 확인했다.

**이 버그가 만든 잘못된 결론**: "v75 완주(3000)" 로 재서 v77 과 비교한 표가
있었는데 그 측정 대상은 실제로 **200 iter 짜리 맨바닥 정책**이었다. 거기서
나온 "마지막 200 iter 가 v75 의 흔들림을 나쁘게 만들었다" 는 근거가 없다.

### 장기 학습 — 리워드 곡선을 수렴 판정에 쓰면 안 된다

v75 를 2800 -> 11800 iter 로 늘렸다. 리워드는 3700 이후 거의 안 늘었다
(1000 iter 당 +1~7, 초반은 +44). **리워드만 보면 수렴이다.** 실제로 나는
"평평하니 멈추자" 고 판단했고 그게 틀렸다:

    지표          @2800    @11800    변화
    roll 속        46.2      31.0    -33 %
    roll RMS       2.85      2.25    -21 %
    낙상 %          0.2       0.0    -100 %
    앞뒤 추종     0.0228    0.0092    -60 %
    회전 추종     0.0320    0.0148    -54 %
    옆 추종       0.0370    0.0429    +16 %   <- 유일한 후퇴
    포화 %         0.20      0.20     변화 없음
    일률 W          6.6       6.4     -4 %

**왜 리워드가 안 잡았나**: 리워드의 대부분인 `imitation`(23 %)과
`path_tracking`(32 %)이 이미 포화됐고, 남은 개선은 `torso_ang_vel` 같은
**작은 벌점 항**에서 일어난다. 그 항들의 절대 크기가 0.01~0.03 이라 총합
450 에서 보이지 않는다.

**교훈**: 수렴 판정은 리워드가 아니라 **목표 지표로** 한다. `odm measure` 를
중간 체크포인트에도 돌려야 한다.

토크 쪽(포화·일률)은 장기 학습으로 안 는다. 그건 v74/v75 의 리워드 설계에서
이미 다 얻었다.

### LPF 스윕 — 그리고 장기 학습이 그것을 대체한다

`ODM_LPF` 로 액션 저역통과를 걸고 v75 를 재측정했다. alpha 는
`filt = alpha*filt + (1-alpha)*new` 라 클수록 강한 평활이다.

    alpha  차단 Hz   포화%   일률W   roll속   rollRMS   낙상%   앞뒤     방향
    0.0      —       0.20    6.6     46.2     2.85      0.2   0.0228   OK
    0.3     9.58     0.13    5.9     33.1     2.65      0.0   0.0300   OK
    0.5     5.52     0.08    5.0     67.2     4.23      1.1   0.0304   OK
    0.7     2.84     0.05    3.4     56.5     3.68      0.8   0.0553   FAIL(left.vy)

경계가 물리로 설명된다. 이 로봇의 보행 주파수는 **1.85 Hz**(주기 540 ms)다.
0.3 은 차단이 보행의 5 배 위라 떨림만 깎지만, 0.7 은 보행 주파수 바로 위라
**보행 동작 자체를 깎는다.** 0.7 에서 토크가 준 것은 부드러워져서가 아니라
덜 움직여서다 — 그래서 추종이 같이 무너지고 낙상이 4 배가 됐다.

**그런데 @11800 에서 다시 재니 LPF 가 손해였다:**

    v75 @11800   필터없음   LPF 0.3
    roll 속        31.0      35.7    <- 필터가 오히려 나쁘게
    회전 추종     0.0148    0.0352    <- 2.4 배 악화
    포화 %         0.20      0.14
    일률 W          6.4       5.9

덜 학습된 정책은 고주파로 떨고 있었고 필터가 그것을 깎아 줬다. 충분히 학습된
정책은 그 떨림이 이미 없어서 **필터가 유효 신호만 깎는다.**

**결론**: 실기 기본값은 **필터 없음(0.0)**. 실기에는 심에 없는 노이즈(센서·버스
지터·기구 유격)가 있으니, 실제로 떨면 그때 0.3 을 켜는 카드로만 남긴다.

### 현재 최고 정책

**v75 @11800** (`2026-08-21_03-36-14_imitation_v75/model_11100.pt`)

    포화 0.20 % · 일률 6.4 W · p99 토크 3.15 · roll속 31.0 · rollRMS 2.25
    낙상 0.0 % · 점수 0.0170 · 6 방향 부호 전부 OK

낙상률이 0.0 % 라 이 지표로는 더 이상 정책을 못 가른다. 앞으로는 외란을 준
상태에서 재거나 측정을 더 길게 해야 한다.

### 진행 중이던 것 (오늘 중단)

    v77  2026-08-21_12-31-24  model_8400.pt  (재개로 3000 -> 8400)

v77 은 v75 와 같은 궤적을 그리고 있었다 (iter 8400 에서 리워드 466 / 에피 806,
v75 는 8700 에서 461 / 799). 마찰 0.4~1.3 이라는 더 넓은 조건에서도 성능이
같다는 뜻이다. 측정은 아직 안 했다.

### 남은 것

1. **실기 배포** — v75 @11800, 필터 없이. 심 순위와 실기 순위가 어긋난 전례가
   있어 여기서 확인해야 한다
2. **실물 발바닥 TPU 돌출 여부** — 심-실기 좌우 접지폭 2.4 배 차이 의혹
