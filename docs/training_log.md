...
## v50(Rough) — 평지/자갈/자갈돌 혼합 지형 + 정지 hip_inward 수정 (2026-08-13)

사용자 요청: 자갈밭·험지에서도 잘 걷게. main이 open-duck-mini-isaaclab을
흡수 병합(v49까지)한 뒤라 v49 위에 지형만 얹었다 — `terrain.py`
(IsaacLab TerrainGeneratorCfg, flat 50%/gravel 25%/rocks 25%, 이 로봇
크기에 맞춰 ANYmal급 표준 예제보다 스케일 축소), 병렬 환경 절반씩 동시
학습(순차 커리큘럼 아님).

### 부수 발견 — 정지 시 다리-몸통 충돌이 심각했다

첫 학습(v43, 4096env, 3000iter) 실측: 5mm 안전 위반이 **정지에서 88.0%,
실접촉 46.0%**(그 전까지 기록된 최악은 v25의 56.7%였는데 그건 보행 중이었다).
같은 체크포인트를 지형 없는 태스크(v42 기준)에 얹어 재생해도 재현(위반
86.0%/접촉 82.0%, 오히려 더 나쁨) — **지형과 무관한 기존 결함**임을 확인.

원인 추적: v38이 "정지에서 hip_inward를 통째로 끈다"로 좌우 비대칭을
해결했었는데(레퍼런스가 위상 0의 비대칭 자세를 기준으로 삼아 leg_symmetry와
싸우던 문제), 그 판단의 전제("정지 자세는 실측 62mm 여유")가 v41(DCMotor
액추에이터)·v42(대칭 레퍼런스로 READY 자세 교체)를 거치며 조용히 깨졌다.
`joystick_env.py`에서 "정지에서 끈다" 대신 "정지에서는 레퍼런스(비대칭)
대신 정지 목표 자세(대칭 — cost_stand_still이 쓰는 것과 동일)를 기준으로
잰다"로 수정 — leg_symmetry와 더 이상 안 싸운다.

### 결과 (v51 재학습, 4096env·3000iter·1시간57분, head_bob 포함 — 아래 참고)

6방향 추종(odm measure, model_2999):
...
**정지 위반 88.0%→0.0%, 접촉 46.0%→0.0%.** hip_inward 수정이 정확히
의도대로 작동했다. 우/회전에 약간의 위반(8%/4%)이 남지만 전 방향 실접촉은
0%.

### 남은 것

* 우/회전의 잔여 5mm 위반(8%/4%, 접촉은 0%) — 원인 미조사
* v50(지형만, head_bob 없음) 단독 수치는 못 얻음 — v43(버그 있는 채로)만
  측정했고, 수정 후 첫 학습이 이미 head_bob이 섞인 v51이었다. 필요하면
  v50만 다시 학습해서 head_bob 효과를 분리할 것.

---

## v51(Rough2) — 보행 중 목(neck_pitch) 위상 동기 끄덕임 (2026-08-13)

사용자 요청: "목 부분이 보행할 때만 주기에 맞춰 살짝 위아래로, 1cm 높이로".

placo 레퍼런스 모방(다리와 같은 방식)을 먼저 검토했으나, 원본 세 저장소
(apirrone/Open_Duck_Mini, Open_Duck_Playground, Open_Duck_reference_motion_
generator)를 전부 확인한 결과 목 동작 레퍼런스가 어디에도 없음을 확인 —
placo는 다리만 풀고, Playground의 `cost_head_pos`는 정의만 되고 실제
리워드 딕셔너리에 연결된 적이 없는 죽은 코드. 이 컴퓨터엔 placo가 안
깔리고(빌드 실패) 레퍼런스를 만들던 랩PC도 지금 네트워크가 달라 접속
불가라, 위상 기반 절차적 목표(사인파)로 구현.

`rewards.reward_head_bob`: `target = base_angle + amplitude*sin(phase)`,
phase는 imitation_phase 관측과 같은 신호 공유, `cmd_norm > 0.01` 게이트로
정지 시 꺼짐. `neck_pitch` 하나만 `lock_head_joints` 예외 처리, 나머지
3개 머리 관절은 그대로 고정.

**진폭 0.2618 rad(15도)는 pinocchio FK 실측값** — Z자 목 READY(30도) 기준
±15도 스윙 시 "head" 프레임 월드Z가 9.64mm 이동(요청한 1cm에 가장 근접,
관절 체인이 비선형이라 스윕으로 검증). `head_bob_scale=1.0`은 첫 시도값.

### 결과

전진 롤아웃(300스텝) 직접 확인 — `neck_pitch`가 보행 주기(27스텝)에
정확히 phase-lock돼 오르내림(그래프로 확인, 회색 세로선=주기 경계와
정확히 일치). 다만 **진폭이 목표에 못 미친다**: 목표 peak-to-peak 30도인데
실측은 약 24도(±12도) — FK 환산하면 약 7mm(목표 10mm의 70~80%). RL이 다른
리워드 항과 타협한 결과로 보인다.

걷기 성능(6방향 추종·5mm 안전)은 위 v50 절 참고 — head_bob 추가가 걷기를
해치지 않았다(오히려 v43 대비 개선, 단 hip_inward 수정 효과와 섞여있어
head_bob 단독 기여는 분리 안 됨).

### 남은 것

* 진폭 부족(목표의 70~80%) — `head_bob_scale`을 1.0보다 올려서 재학습하면
  개선되는지 확인 필요
* head_bob 단독 효과(지형 수정과 분리) 측정 안 됨 (위 v50 절 참고)
* 랩PC 접속 되면 placo 기반 진짜 레퍼런스로 교체하는 옵션 남아있음
  (지금 구조 — 목표각 vs 실측각 추종 — 그대로 재사용 가능)

---

## v52~v54(Rough3~5) — head_bob 반복 튜닝 (2026-08-13, 조이스틱 실측 기반)

사용자가 매 버전 조이스틱으로 직접 테스트하며 아래를 순차 반영:

* **Z자 목 고정 커플링** (Rough3): `head_pitch`를 `neck_pitch`의 델타에
  음수로 하드 커플링(`head_bob_counter_joint`) — RL이 배우게 두지 않고
  액션 레벨에서 매 스텝 강제. "리워드로는 보장이 안 된다"는 프로젝트
  기조 그대로 적용.
* **정지 시 목 처짐 수정** (Rough4): 원래 `reward_head_bob`이
  `cmd_norm<=0.01`일 때 리워드 전체를 0으로 게이트해서, 걷다가 멈추면
  스윙 도중 각도에서 그레디언트를 잃고 그대로 처져 있었다. 사인의
  진폭만 0으로 접어서(목표가 `base_angle`로 수렴) 정지 시에도 살아있는
  그레디언트를 유지하도록 리워드를 다시 씀.
  진폭도 15도→8.6도로 축소.
* **진폭 10도 상한 확정** (Rough5): 사용자가 "더 적게 움직이는 게 좋겠다"
  는 판단 후 8.6도→10도 사이에서 10도로 최종 확정. 이 시점에 `head_bob`
  리워드를 처음부터 다시 작성 — neck_pitch/head_pitch만 사용, 동작은
  "Z자 두 각이 같이 제어되는 붕 뜨고 가라앉는" 병진 운동이라는 사용자
  설명을 반영해 기존 Z-커플링 설계가 맞았음을 재확인.

세 버전 모두 개별 정량 측정(odm measure, leg_trunk_clearance)은 생략하고
바로 다음 버전으로 넘어감 — 조이스틱 정성 테스트만으로 다음 결정을 내림.

---

## v55(Rough6) — 목 바운싱 주기를 '걸음마다 1번'으로 (2026-08-14)

사용자 질문: "걸을 때 오른발이 나갈 때 바운싱 한번 맞아?" — 확인해보니
기존 `sin(phase)`는 `gait_period_steps`(27스텝) 한 바퀴, 즉 **양발 왕복
스트라이드당 1번**이었다 (레퍼런스 `ref_g125sym.pkl`의 발 접지 채널로
확인: phase 40~120°가 오른발 스윙, 130~240°가 왼발 스윙 — 한 주기 안에
둘 다 들어있음). 사용자가 "걸음마다로 바꿔줘"라고 요청 → `head_bob`
리워드에 `cycles_per_period` 인자를 추가해 `sin(cycles_per_period*phase)`
로 일반화하고 `2.0`으로 설정 (피크가 phase 45°/225°에 위치, 각각 오른발/
왼발 스윙 구간 안에 들어오는 것을 수학적으로 확인 후 반영).

3000 iteration, envs=4096, 2시간 2분 학습. 최종 평균 리워드 265.48,
`imitation` 0.1998, `hip_inward` 페널티 -0.0072(낮음=안전), `head_bob`
0.0095.

### 결과 — 6방향 추종 (`odm measure v55`)

| 명령 | achieved | err |
|---|---|---|
| 정지 | (-0.004,-0.009) | 0.010 |
| 앞 | (+0.134,-0.010) | 0.019 |
| 뒤 | (-0.133,+0.006) | 0.018 |
| 좌 | (+0.018,+0.154) | 0.049 |
| 우 | (-0.021,-0.191) | 0.023 |
| 회전 | (+0.001,+0.005) | 0.005 |

좌측 추종 오차(0.049)가 상대적으로 크지만 전 방향 안정적. imitation
internals: 관절각 오차 8.8도/관절 (기존 정상 범위), base speed 0.161m/s
(레퍼런스 0.194m/s로 약간 느림).

### 결과 — 5mm 안전 클리어런스 (`leg_trunk_clearance.py`)
...
전 방향 위반 0%, 접촉 0% — hip_inward 정지 수정이 v55에서도 계속
유지됨.

### 결과 — 목 바운싱 걸음마다 정합 (전진 300스텝, `check_head_bob_v55.py`)

`neck_pitch` 로컬 피크 23개 검출 (설계상 기대치: 300스텝/27스텝 ×2 ≈
22개, 거의 일치). 그중 22개(96%)가 좌/우 발 중 하나의 스윙 구간
(접지→이지) 안에 위치 — "걸음마다 1번" 설계가 실측에서도 확인됨.
스윙 구간 자체(35개, 좌15/우20)로 나눈 스윙당 평균 피크 수는 0.63으로
1.0에 못 미치는데, 좌/우 스윙 횟수가 15:20으로 비대칭인 것을 보면 발
접지 신호 노이즈로 스윙 구간이 과다 분할(허상 스윙)됐을 가능성이 커서,
96% 겹침 쪽이 더 신뢰할 수 있는 지표로 판단. 필요하면 접지 임계값을
올리거나 최소 스윙 길이 필터를 추가해 스윙 검출 자체를 다시 검증할 것.

### 남은 것

* v50(head_bob 없음) 단독 수치 여전히 미측정 — head_bob이 걷기 성능에
  주는 순수 영향은 아직 분리 안 됨
* 좌측 추종 오차(0.049)가 다른 방향 대비 큼 — 원인 미조사
* 스윙 검출 노이즈(좌우 비대칭 15:20) — 접지 신호 임계값/필터 재검토
  여지 있음
